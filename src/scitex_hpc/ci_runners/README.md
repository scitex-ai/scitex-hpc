...
scitex-hpc ci-runners gen-archive --out /tmp/archive-bandaids.sh
```
...
6. `scitex-hpc ci-runners gen-monitor --out ~/.scitex/ci/monitor.sh`,
   then add to crontab:
   ```
...
7. Wire the alarm: either set `SCITEX_CI_ALARM_CMD` in the crontab env
   ...
   the stderr.
8. `scitex-hpc ci-runners gen-archive` and **hand-run** the result on
